...
const StyledSVG = styled.svg<{ phased: boolean, all: string[], phases: string[][], phase?: number }>`
      #form {
  ...
  #page {
      transform: translateX(200%);
      animation: printer-in 1s ${({ phased, phase }) => phased && phase == 2 ? '0s' : '13.5s'} forwards,print-page 1s ${({ phased, phase }) => phased && phase == 2 ? '2s' : '15.5s'} forwards, page-move-up 1s ${({ phased, phase }) => phased && phase == 2 ? '4.5s' : '17.5s'} forwards, page-move-left 1s 20.5s forwards, page-out 1s ${({ phased, phase }) => phased && phase == 2 ? '7s' : '30s'} forwards; 
  }
      #signature {
        font-size: 0px;
      ...
      #printer,#printer_front {
        transform: translateX(200%);
      animation: printer-in 1s ${({ phased, phase }) => phased && phase == 2 ? '0s' : '13.5s'} forwards, printer-out 1s ${({ phased, phase }) => phased && phase == 2 ? '4s' : '17.5s'} forwards;
  }
      #phone {
        transform: translateX(-200%);
      animation: phone-in 1s ${({ phased, phase }) => phased && phase == 1 ? '0s' : '5s'} forwards, phone-out 1s ${({ phased, phase }) => phased && phase == 1 ? '8s' : '13s'} forwards;
  }
      #terminal {
        transform: translateY(200%);
      animation: terminal-in 1s ${({ phased, phase }) => phased && phase == 1 ? '0s' : '5s'} forwards, terminal-out 1s ${({ phased, phase }) => phased && phase == 1 ? '8s' : '13s'} forwards;
  }
      #terminal_back {
        transform: translateY(200%);
      animation: terminal-in 1s ${({ phased, phase }) => phased && phase == 1 ? '0s' : '5s'} forwards, terminal-out 1s ${({ phased, phase }) => phased && phase == 1 ? '8s' : '13s'} forwards;
      ...
      #card {
        transform: translateY(-200%);
      animation: card-in 2s ${({ phased, phase }) => phased && phase == 1 ? '2s' : '7s'} forwards, card-out 0.5s ${({ phased, phase }) => phased && phase == 1 ? '5.5s' : '10.5s'} forwards;
  }
      #terminal_ok_1 {
        font-size: 0px;
      animation: phone-ok 0.25s ${({ phased, phase }) => phased && phase == 1 ? '6s' : '11s'} forwards;
  }
      #terminal_ok_2 {
        font-size: 0px;
      animation: phone-ok 0.25s ${({ phased, phase }) => phased && phase == 1 ? '6.25s' : '11.25s'} forwards;
  }
      #phone_ok_1 {
        font-size: 0px;
      animation: phone-ok 0.25s ${({ phased, phase }) => phased && phase == 1 ? '1s' : '6s'} forwards;
  }
      #phone_ok_2 {
        font-size: 0px;
      animation: phone-ok 0.25s ${({ phased, phase }) => phased && phase == 1 ? '1.25s' : '6.25s'} forwards;
  }
      @keyframes sign {
        0% {
          font-size: 0px;
    ...
    100% {
      ...
      @keyframes page-out {
        0% {
          transform: translate(${({ phased, phase }) => (!phased || phase != 2) ? '-25%' : '0%'}, 15%)${({ phased, phase }) => (phased && phase == 2) ? ' scale(1);' : ';'} 
        }
        100% {
            transform: translate(${({ phased, phase }) => (!phased || phase != 2) ? '-25%' : '-200%'}, 200%)${({ phased, phase }) => (phased && phase == 2) ? ' scale(5);' : ';'} 
        }
      }
   ...
   ${({ phased, all, phases, phase }) => phased ? all.filter(id => !(phases[phase as number].includes(id))).map((id) => `#${id} {display: none; }`).join('\n') : ''}
  `


export const ExplanationAnimationSvg = ({ style, className, phase }: {
...
}) =>
{
	...
	const all = [
		...
		'page', 'terminal', 'card',
		'terminal_back', 'hammer', 'hammer_stand',
		'phone', 'cloud_inner', 'cloud_outer'
	];

	const phases = [
		['form'],
		['phone', 'terminal_back', 'terminal', 'card'],
		['printer', 'printer_front', 'page'],
		['page', 'hammer_stand', 'hammer', 'cloud_inner', 'cloud_outer'],
	]


	return <>
		<StyledSVG  {...{ phased, all, phases, phase }} className={className} width='100%' viewBox="0 0 1179 796" fill="none" xmlns="http://www.w3.org/2000/svg">
			<g id="form">
				<g id="Rectangle 1">
					<path id="Vector" d="M29 208H1149V704C1149 709 1145 714 1139 714H39C33 714 29 709 29 704V208Z" fill="#E2E8F0" />
				</g>
				<g id="Rectangle 2">
					<path id="Vector_2" d="M29 88C29 82 33 78 39 78H1139C1145 78 1149 82 1149 88V208H29V88Z" fill="#3B82F6" />
				</g>
				<g id="Rectangle 3">
					<path id="Vector_3" d="M726 109H78C70 109 63 116 63 124V161C63 169 70 176 78 176H726C734 176 741 169 741 161V124C741 116 734 109 726 109Z" fill="#E2E8F0" />
				</g>
				<g id="Rectangle 4">
				...
				<g id="Group 1">
					...
					<path id="Rectangle 10" d="M553 351H325C319 351 315 355 315 361V393C315 399 319 403 325 403H553C558 403 563 399 563 393V361C563 355 558 351 553 351Z" fill="#E2E8F0" />
					<path id="Rectangle 11" d="M852 351H624C618 351 614 355 614 361V393C614 399 618 403 624 403H852C857 403 862 399 862 393V361C862 355 857 351 852 351Z" fill="#E2E8F0" />
					<path id="Rectangle 13" d="M853 575H323C318 575 313 580 313 585V638C313 644 318 648 323 648H853C859 648 863 644 863 638V585C863 580 859 575 853 575Z" fill="#E2E8F0" />
					<path id="Rectangle 12" d="M855 446H627C621 446 617 451 617 456V489C617 494 621 499 627 499H855C860 499 865 494 865 489V456C865 451 860 446 855 446Z" fill="#E2E8F0" />
				</g>
			...
			<g id="phone">
				<g id="Rectangle 27">
					<path id="Vector_27" d="M42 65C42 59 46 55 52 55H521C526 55 531 59 531 65V733C531 738 526 743 521 743H52C46 743 42 738 42 733V65Z" fill="#E2E8F0" />
				</g>
				<g id="Rectangle 28">
				...
				<g id="Rectangle 21_2">
					<path id="Vector_30" d="M406 101H100C94 101 90 105 90 111V137C90 142 94 147 100 147H406C412 147 416 142 416 137V111C416 105 412 101 406 101Z" fill="#E2E8F0" />
				</g>
				<g id="Rectangle 27_2">
			...
			<g id="card">
				<g id="Rectangle 33">
					<path id="Vector_41" d="M892 59H552C546 59 542 63 542 69V619C542 625 546 629 552 629H892C898 629 902 625 902 619V69C902 63 898 59 892 59Z" fill="#E2E8F0" />
				</g>
			...
			<g id="terminal">
				...
				<g id="Rectangle 40">
					<path id="Vector_46" d="M905 659H842C840 659 837 662 837 664V694C837 697 840 699 842 699H905C907 699 910 697 910 694V664C910 662 907 659 905 659Z" fill="#E2E8F0" />
				</g>
				<g id="Rectangle 41">
					<path id="Vector_47" d="M1082 659H1020C1017 659 1015 662 1015 664V694C1015 697 1017 699 1020 699H1082C1085 699 1087 697 1087 694V664C1087 662 1085 659 1082 659Z" fill="#E2E8F0" />
				</g>
				<g id="Rectangle 42">
					<path id="Vector_48" d="M993 659H931C928 659 926 662 926 664V694C926 697 928 699 931 699H993C996 699 998 697 998 694V664C998 662 996 659 993 659Z" fill="#E2E8F0" />
				</g>
				<g id="Rectangle 43">
			...
			<g id="printer">
				<g id="Rectangle 52">
					<path id="Vector_58" d="M1093 302H86C80 302 76 306 76 312V598C76 603 80 608 86 608H1093C1098 608 1103 603 1103 598V312C1103 306 1098 302 1093 302Z" fill="#E2E8F0" />
				</g>
				<g id="Rectangle 53">
			...
			<g id="printer_front">
				...
				<path id="Rectangle 65_2" d="M1093 302H86C80 302 76 306 76 312V417C76 423 80 427 86 427H1093C1098 427 1103 423 1103 417V312C1103 306 1098 302 1093 302Z" fill="#E2E8F0" />
				<g id="Rectangle 54">
