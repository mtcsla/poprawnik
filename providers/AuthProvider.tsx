...
const AuthProvider = ({ children }: { children: React.ReactNode }) =>
{
	...
	useEffect(() =>
	{
		...
		if (!userProfile)
			...
			{
				setRedirecting(true)
				router.replace('/login').then(() => setRedirecting(false));
				return;
			}
	...
	return (
		<authContext.Provider value={{ user, userProfile, signIn, signUp, signOut, errorCodes }}>
			...
			>
				<DialogTitle>
					<pre>Wylogowujesz się</pre>
				</DialogTitle>
				<DialogContent>
					<BodyScrollLock>
						<p className="text-base">Czy na pewno chcesz się wylogować?</p>
					</BodyScrollLock>
				</DialogContent>
				<DialogActions>
					<LoadingButton className="bg-blue-100" onClick={async () =>
					{
						await logOut(auth);
					...
					<LoadingButton color='error' className="bg-red-100" onClick={() =>
					{
						setConfirmLogoutOpen(false);
					}}>
						Anuluj
